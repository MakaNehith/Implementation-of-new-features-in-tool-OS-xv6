...
test_pa4_11(void)
{
    ...
    diskstat.disk_reads = 0;
    diskstat.disk_writes = 0; // reset stats
    ...
    printf("\nDisk Stats (SSTF): Reads=%d, Writes=%d, Avg Latency=%ld.%ld ticks\n",
           diskstat.disk_reads, diskstat.disk_reads, diskstat.avg_disk_latency / 100, diskstat.avg_disk_latency % 100);
